...
export default function EditorsChoicePage() {
  ...
  const fetchArticles = async (page: number) => {
    try {
      setLoading(true);
      const response = await fetch(`/api/articles?type=editors-choice&page=${page}&limit=12`);
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="container mx-auto !px-4 !py-8">
        <div className="border-l-4 border-orange-500 pl-6 mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Editor's Choice</h1>
          <p className="text-gray-600 mt-2">Handpicked articles by our editorial team</p>
          {pagination && (
                ...
                <article key={article.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                  <Link href={`/news/${article.id}`} className="group">
                    <div className="relative aspect-[16/10] overflow-hidden">
                      ...
                      <div className="absolute top-3 right-3 bg-orange-600 text-white px-2 py-1 text-xs font-medium rounded">
                        Editor's Choice
                      </div>
                    </div>
        ...
        {!loading && articles.length === 0 && (
          <div className="text-center py-12">
            <h3 className="text-xl font-semibold text-gray-900 mb-2">No Editor's Choice Available</h3>
            <p className="text-gray-600">Check back later for our editor's picks!</p>
          </div>
        )}
      </div>
    </main>
